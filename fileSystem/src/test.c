...
////	inicio_mock();
	...
	alta_nodo_viejo("PC23");
	alta_nodo_viejo("PC231");
	alta_nodo_viejo("PC232");

	consola();

}

void procesarComando(t_log* logger) {
	char word_aux[256];
			...
			if (strcmp(comandos[0], "dir") == 0) {
			...
			} else if (strcmp(comandos[0], "start") == 0) {
				if (strcmp(comandos[1], "nodo") == 0) {
					if (comandos[2] != NULL) {
						alta_nodo_viejo(comandos[2]);
					} else {
						txt_write_in_stdout("Debe indicar un nombre para el nodo.\n");
					}
				}
			} else if (strcmp(comandos[0], "stop") == 0) {
				if (strcmp(comandos[1], "nodo") == 0) {
					if (comandos[2] != NULL) {
			...
			} else if (strcmp(comandos[0], "copy") == 0) {
				if (strcmp(comandos[1], "file") == 0)
					if (comandos[2] != NULL)
						copiar_archivo_a_mdfs(comandos[2], comandos[3], logger);
			...
			} else if (strcmp(comandos[0], "delete") == 0) {
				if (strcmp(comandos[1], "bloque") == 0) {
					if (comandos[2] != NULL && comandos[3] != NULL)
				...
				if(strcmp(formatear_confirmacion, "s") == 0)
					formatear_mdfs();
			} else if (strcmp(comandos[0], "ls") == 0) {
				if (comandos[1] == NULL)
					listar_todo();
				else {
