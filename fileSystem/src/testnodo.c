...
int tratarMensaje(int numSocket, header_t* mensaje, void* extra, t_log* LOGGER){
	...
	switch(mensaje->tipo){
		...
		case NODO_TO_FS_GET_BLOQUE_OK:
			log_info(logger, "Mensaje recibido: [%s] del socket [%d]", getDescription(mensaje->tipo), numSocket);
			recibirNodoToFSGetBloque(numSocket, mensaje, logger);
			break;
...
int enviarFSToNodoGetBloque(int socketNodo, t_log* logger, int nroBloque){
	...
	log_info(logger, "Enviando header GET_BLOQUE al nodo por el socket [%d] [%s]\n", socketNodo, getDescription(header.tipo));

	if (enviar_header(socketNodo, &header) != EXITO)
	{
		log_error(logger, "Error enviando GET_BLOQUE al nodo por el socket [%d] [%s]\n", socketNodo, getDescription(header.tipo));
		return ERROR;
	}
	...
	log_info(logger, "Enviando GET_BLOQUE[nro_bloque: %d] al nodo por el socket [%d] [%s]\n", nroBloque, socketNodo, getDescription(header.tipo));

	if (enviar_struct(socketNodo, &getBloque, sizeof(t_nro_bloque)) != EXITO)
	{
		log_error(logger, "Error enviando GET_BLOQUE[nro_bloque: %d] al nodo por el socket [%d] [%s]\n", nroBloque, socketNodo, getDescription(header.tipo));
		return ERROR;
	}
...
int recibirNodoToFSGetBloque(int socketNodo, header_t* header, t_log* logger){
	...
	{
		log_error(logger,"recibirNodoToFSGetBloque: Error al recibir struct getBloque \n\n");
		return ERROR;
	}
	...
	if (ret != EXITO) {
		...
		log_error(logger, "Error recibiendo GET_BLOQUE[nro_bloque: %d] al nodo por el socket [%d] [%s]\n", nroBloque, socketNodo, getDescription(header->tipo));

		return ret;
	}

	log_info(logger, "recibirNodoToFSGetBloque: contenido recibido de bloque solicitado nro: %d \n", getBloque.nro_bloque);
...
int mainAlternativo(int argc, char *argv[]){
	...
	logger = log_create("logFileSystem.log", "FileSystem", true, LOG_LEVEL_DEBUG);

	char* pathArchiConf = "/home/utnso/dev/tp-2015-1c-los-salieris/fileSystem/src/configFileSystem";
...
//	}
	...
	// FIN - Carga de configuracion del personaje
		...
		//escucharNuevasConexiones(puertoEscucha);
